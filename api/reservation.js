...
module.exports = async (req, res) => {
    try {
      if (req.method === "GET") {
        const { room_id, room, date, userid} = req.query;
        ...
        if (room_id) {
          ...
          return await handlefetchReservations(req, res);
            ...
            return await handlefetchBookingDetails(req, res);
        } else {
    ...
    //     if (user_id && fname && lname && reserve_date && time && businessunit && branch && roomID && roomName && setup && guest && contact && email && table && chair && hdmi && extension && message) {
    //         return await handleReserveRoom(req, res);
    //     }
      } else {
...
const handlefetchBookingDetails = async (req, res) => {
    try {
        if (req.method === 'GET') {
...
//                                           <a href="https://flldc-booking-app.vercel.app/submits" style="background-color:red;color:white;padding:12px 24px;text-align:center;text-decoration:none;border-radius:4px;font-size:16px;display:inline-block">Cancel Booking</a>
//                                         </p>
//                                       </div>
//                                     </td>
//                                   </tr>
